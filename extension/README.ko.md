...
```text
1. build Makefile이 있으면 해당 위치 목록 중 현재 프로젝트 테스트만 맞춥니다.
...
5. 체크박스를 연속으로 선택할 때 실제 refresh가 필요하기 전까지 이미 인식한 테스트 데이터를 재사용합니다.
```

`Pintos` 뷰와 `pt` / `pintos-tests` 터미널 명령은 같은 bundled helper를 사용합니다. 두 경로 모두 build Makefile이 있으면 최종 `TESTS` 목록을 우선 사용하되 현재 사이드바 프로젝트가 소유한 테스트만 남기고, 첫 build 전에는 해당 프로젝트의 `Make.tests`를 fallback으로 읽으며, 같은 Pintos build tree를 거쳐 같은 `output`, `result`, `errors` artifact를 확인합니다.
